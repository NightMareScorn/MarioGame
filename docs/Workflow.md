...
- Xác định mã Issue (VD: `#5`, `#7`) và các ràng buộc kỹ thuật.

### Bước 2: Khởi tạo Nhánh (Branching)
- Luôn cập nhật code mới nhất từ nhánh `test` trước khi tạo nhánh mới.
...
- Truy cập GitHub, tạo PR từ nhánh vừa push vào nhánh `test`.
...
5. **Naming Convention:** 
    ...
    - Function: `PascalCase` (VD: `UpdatePosition()`).
    - Variable:  `camelCase` (VD: `isJumping`).
