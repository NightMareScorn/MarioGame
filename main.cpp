...
void HandleDebugInput() {
	...
	if (kb->IsKeyPressed('A')) {
		DebugOut(L"[TEST] A JUST PRESSED\n");
	}
	if (kb->IsKeyPressed('D')) {
		DebugOut(L"[TEST] D JUST PRESSED\n");
	}

	if (kb->IsKeyDown('D')) {
		DebugOut(L"[TEST] Holding D\n");
	}
	if (kb->IsKeyDown('A')) {
		DebugOut(L"[TEST] Holding A\n");
	}
	...
	if (kb->IsKeyReleased('D')) {
		DebugOut(L"[TEST] D RELEASED\n");
	}
	if (kb->IsKeyReleased('A')) {
		DebugOut(L"[TEST] A RELEASED\n");
	}
